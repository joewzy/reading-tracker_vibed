...
export async function GET() {
    ...
    try {
        ...
        const sessions = await (prisma as any).readingSession.findMany({
            where: { userId: userId },
            orderBy: { date: 'desc' },
            take: 20,
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const sessionEntry = await (prisma as any).readingSession.create({
            data: {
                bookId,
        ...
        const sessionCount = await (prisma as any).readingSession.count({ where: { userId: userId } })

        // Calculate streak for achievement check
        const allSessions = await (prisma as any).readingSession.findMany({
            where: { userId: userId },
            orderBy: { date: 'desc' },
            take: 30,
        })
        ...
        for (let i = 0; i < 30; i++) {
            ...
            const hasSession = allSessions.some((s: any) => new Date(s.date).toDateString() === day.toDateString())
